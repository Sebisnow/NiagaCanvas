...
public class OrderedAggregate extends AbstractOperator {
   ...
   /**
    * Constructs a new aggregation operator for ordered streams with the given input schema that applies the
    * given aggregation functions to the given aggregate attributes based on the given grouping attributes.
    *
    * @param inputSchema
    *           input schema
    ...
    * @param sortValues
    *           <code>1</code> if the values should be sorted in descending order, <code>2</code> if ascending
    *           order, <code>0</code> otherwise
    * @param limit
    ...
    */
   public OrderedAggregate(final Schema inputSchema, final int[] groupAttributes,
         final int[] aggregatedAttributes, final int sortValues, final int limit,
         final int attributePosition, final AggregationFunction... functions) {
      this(OrderedAggregate.class.getSimpleName(), inputSchema, groupAttributes, aggregatedAttributes,
            sortValues, limit, attributePosition, functions);
   }
   ...
   public OrderedAggregate(final String operatorId, final Schema inputSchema, final int[] groupAttributes,
      ...
      if (this.arrayContains(groupAttributes, progressingAttibute)
         ...
         throw new IllegalArgumentException(
               "The progressing attribute cannot be used in groups or aggregates.");
      }
   ...
   public Schema getOutputSchema() {
      ...
      final GroupedStreamSegment segment = new GroupedStreamSegment(inputSchemas.get(0),
            this.groupAttributes, this.aggregatedAttributes, this.functions);
    ...
    * @param key
    *           integer key
    * @return <code>true</code> if the array contains the key, <code>false</code> otherwise
    */
   ...
   /**
    * Reports the given segment by pushing the aggregated result tuple to the stream and removing the segments
    * from the list of currently processed segments.
    *
    * @param l
    *           segment number
    */
    ...
    * @param descending
    *           <code>1</code> if the values should be sorted in descending order, <code>2</code> otherwise
      ...
      return new Comparator<DataTuple>() {
         ...
         public int compare(final DataTuple first, final DataTuple second) {
            if (first.getSchema().getAttribute(attributePosition).getType().equals(String.class)
               ...
               return ((String) first.getAttributeValue(attributePosition)).compareTo((String) second
                     .getAttributeValue(attributePosition));
            }
   ...
   public static class Factory implements PlanOperatorByParametersFactory {
      ...
      public OrderedAggregate getOperatorByParameters(final String operatorId, final Schema inputSchema,
         ...
         for (final Parameter param : funcParam.getList()) {
            ...
            funcFuncs[counter] = BuiltInAggregationFunction.forType(Type.valueOf(param.getName()
                  .toUpperCase()));
            counter++;
         }
         int limit = 0;
         ...
         if (sortParam != null) {
            ...
            if (sortAttribute == -1) {
               // sort attribute is in generated attributes, iterate over functions
               for (int i = 0; i < funcAttributes.length; i++) {
                  // This is rather ugly because we assume the naming of generated attributes
                  // will never change.
