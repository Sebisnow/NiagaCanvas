...
/**
 * This opens an Option Window for a NiagarinoOperator to set its values in a
 * GUI. To improve style implement as Modal JDialog like JOptionpane.
 * 
 * @author sebastian
 *
 */
...
public class OptionWindow extends JFrame {
	...
	/**
	 * Reference to a JTextField to be able to call it from without the specific
	 * methods.
	 */
	private JTextField textField;

	/**
	 * Reference to a Creation Button to be able to call it from without the
	 * specific methods.
	 */
	private JButton button;

	/**
	 * A NiagarinoOperator whose options are represented by this Window.
	 */
	private final NiagarinoOperators op;

	/**
	 * The Option Window itself
	 */
	private OptionWindow self;

	/**
	 * The current view to get all the view data.
	 */
	private SimpleNodeLinkView<AnimatedPosition> view;

	/**
	 * The NodeLink that initiated this Window, to allow access to
	 * DefaultNodeLinkRenderpass and NodeLink.
	 */
	 ...
	 * 
	 */
	public OptionWindow(Stream operator, SimpleNodeLinkView<AnimatedPosition> view2, NodeLink link) {
		this.link = link;
		this.self = this;
		this.view = view2;
		this.op = operator;
		...
		gbc_lblOperatorType.anchor = GridBagConstraints.SOUTHEAST;
		...
		JLabel startlbl = new JLabel("The Start Operator:");
		GridBagConstraints gbc_startlbl = new GridBagConstraints();
		gbc_startlbl.insets = new Insets(5, 5, 5, 5);
		gbc_startlbl.anchor = GridBagConstraints.EAST;
		gbc_startlbl.gridx = 0;
		gbc_startlbl.gridy = 4;
		getContentPane().add(startlbl, gbc_startlbl);

		JComboBox<String> startCombo = new JComboBox<String>();
		startCombo.addItem(" ");
		for (Operator node : operator.getOperatorList()) {
			if (node != operator.getEnd()) {
				startCombo.addItem(node.getID() + " " + node.getName());
				if (operator.getStart() == node) {
					this.checkStart(operator);
					startCombo.setSelectedIndex(startCombo.getItemCount() - 1);
				}
			}
		}
		...
		gbc_start.anchor = GridBagConstraints.WEST;
		gbc_start.gridx = 1;
		gbc_start.gridy = 4;
		this.getContentPane().add(startCombo, gbc_start);

		JLabel endlbl = new JLabel("The End Operator:");
		GridBagConstraints gbc_endlbl = new GridBagConstraints();
		gbc_endlbl.insets = new Insets(5, 5, 5, 5);
		gbc_endlbl.anchor = GridBagConstraints.SOUTHEAST;
		gbc_endlbl.gridx = 0;
		gbc_endlbl.gridy = 5;
		getContentPane().add(endlbl, gbc_endlbl);

		JComboBox<String> endCombo = new JComboBox<String>();
		endCombo.addItem(" ");
		for (Operator node : operator.getOperatorList()) {
			if (node != operator.getStart()) {
				endCombo.addItem(node.getID() + " " + node.getName());
				if (operator.getEnd() == node) {
					this.checkStart(operator);
		...
		gbc_end.insets = new Insets(0, 0, 0, 5);
		gbc_end.anchor = GridBagConstraints.WEST;
		gbc_end.gridx = 1;
		gbc_end.gridy = 5;
		this.getContentPane().add(endCombo, gbc_end);
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// evaluate the startCombo and set streams start.
				if (startCombo.getSelectedItem().equals(" ")) {
					operator.setStart(null);
				} else {
					String[] subStr = ((String) startCombo.getSelectedItem()).split(" ");
					link.setStart(operator, (Operator) view.getNode(Integer.parseInt(subStr[0])));
				}
				// evaluate the endCombo and set streams end.
				if (endCombo.getSelectedItem().equals(" ")) {
					operator.setEnd(null);

				} else {
					String[] subStr = ((String) endCombo.getSelectedItem()).split(" ");
					link.setEnd(operator, (Operator) view.getNode(Integer.parseInt(subStr[0])));
				}
	...
	/**
	 * Helper method to make sure within a Stream an Operator has different
	 * Start. If they are the same delete the End reference. and End Operators.
	 * 
	 * @param stream
	 */
	private void checkStart(Stream stream) {
		if (stream.getStart() == stream.getEnd()) {
			stream.setEnd(null);
			isSet = false;
		}
	 ...
	 * 
	 */
	public OptionWindow(Operator operator, SimpleNodeLinkView<AnimatedPosition> view2) {
		this.op = operator;
		this.view = view2;
		...
		getContentPane().add(this.button, gbc_button);
	...
	private void redrawOperator(String opName) {
		...
		for (ParamDescription paramDesc : param) {
			...
			this.getContentPane().add(label, gbc_label);
			...
			case ParamDescription.STRING:
				...
				gbc_text.gridx = 1;
				gbc_text.gridy = cnt;
				this.getContentPane().add(text, gbc_text);
				this.button.addActionListener(new ActionListener() {
			...
			case ParamDescription.BOOLEAN:
				...
				gbc_combo.gridx = 1;
				gbc_combo.gridy = cnt;
				this.getContentPane().add(box, gbc_combo);

				this.button.addActionListener(new ActionListener() {

					@Override
					public void actionPerformed(ActionEvent e) {
						operator.properties.put(paramDesc.getName(), (String) box.getSelectedItem());

						ArrayList<String> predValues = new ArrayList<String>();
						predValues.add((String) box.getSelectedItem());
			...
			case ParamDescription.DOUBLE:
				...
				gbc_text2.gridx = 1;
				gbc_text2.gridy = cnt;
				this.getContentPane().add(text2, gbc_text2);
				...
				this.button.addActionListener(new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						// TODO Working Rudimentary implementation. Receives one
						// Double as
						// input.
						ArrayList<String> predValues = new ArrayList<String>();
			...
			case ParamDescription.LONG:
				...
				gbc_text3.gridx = 1;
				gbc_text3.gridy = cnt;
				this.getContentPane().add(text3, gbc_text3);
			...
			case ParamDescription.PREDICATE:
				...
				gbc_box1.gridx = 1;
				gbc_box1.gridy = cnt;
				this.getContentPane().add(box1, gbc_box1);
				...
				gbc_text5.gridx = 2;
				gbc_text5.gridy = cnt;
				this.getContentPane().add(text5, gbc_text5);
			...
			case ParamDescription.GROUP:
				...
				gbc_text6.gridx = 1;
				gbc_text6.gridy = cnt;
				this.getContentPane().add(text6, gbc_text6);
			...
			case ParamDescription.AGGREGATIONFUNCTION:
				...
				gbc_text7.gridx = 1;
				gbc_text7.gridy = cnt;
				this.getContentPane().add(text7, gbc_text7);
				this.button.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
			...
			case ParamDescription.STRINGFUNCTION:
				...
				gbc_text8.gridx = 1;
				gbc_text8.gridy = cnt;
				this.getContentPane().add(text8, gbc_text8);
			...
			this.getContentPane().add(this.button, gbc_button);
