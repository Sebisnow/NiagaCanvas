...
public class StringDrawer {
	...
	public static enum Orientation {
		...
		/**
		 * Text rotated by 90 degrees counter-clockwise. Reading from bottom to
		 * top.
		 */
		VERTICAL,
	 ...
	 * @see #BOTTOM
	 */
	public Shape getBounds(final Point2D pos, final int hpos, final int vpos, final Orientation o) {
		Shape res;
		switch (o) {
		case HORIZONTAL:
	...
	public Rectangle2D getBounds(final Point2D pos, final int hpos, final int vpos) {
		return new Rectangle2D.Double(pos.getX() + getHorizontalOffset(hpos) + bbox.getX(),
				pos.getY() + getVerticalOffset(vpos, true) + bbox.getY(), bbox.getWidth(), bbox.getHeight());
	}
	 ...
	 * @see #BOTTOM
	 */
	public Shape getRotatedBounds(final Point2D pos, final int hpos, final int vpos, final double theta) {
		final AffineTransform at = AffineTransform.getTranslateInstance(pos.getX(), pos.getY());
		at.rotate(theta);
	...
	public void drawInto(final RectangularShape rect) {
		...
		g.scale(scale, scale);
		g.translate(getHorizontalOffset(CENTER_H), -bbox.getHeight() + getVerticalOffset(CENTER_V, false));
		g.drawString(str, 0, 0);
		g.dispose();
	}
	 ...
	 * @see #BOTTOM
	 */
	public void drawRotated(final Point2D pos, final int hpos, final int vpos, final double theta) {
		final Graphics2D g = (Graphics2D) gfx.create();
		g.translate(pos.getX(), pos.getY());
		g.rotate(theta);
		g.translate(getHorizontalOffset(hpos), getVerticalOffset(vpos, false) - bbox.getHeight());
		g.drawString(str, 0, 0);
		if (jkanvas.Canvas.DEBUG_BBOX) {
			g.setColor(java.awt.Color.RED);
			jkanvas.util.PaintUtil.setAlpha(g, 0.4);
	 ...
	 *            The vertical alignment.
	 * @param isBBox
	 *            Whether the offset is used to compute the bounding box.
	 * @return The offset.
	 */
	 ...
	 *            The rectangle.
	 */
	public static final void drawInto(final Graphics2D gfx, final String text, final RectangularShape rect) {
		final Graphics2D g = (Graphics2D) gfx.create();
	 ...
	 */
	public static final void drawInto(final Graphics2D gfx, final String text, final RectangularShape rect,
			final Orientation o, final int alignX) {
		...
		g.scale(scale, scale);
		// If Node to draw is Stream put Name on top
		if (rect.getHeight() >= RectangularNodeRealizer.RADIUS) {
			sd.draw(ORIGIN, alignX, TOP, Orientation.TOP_E);
		} else {
	 ...
	 *            The orientation.
	 */
	private static void translateLeftAligned(final Graphics2D g, final RectangularShape rect, final Orientation o) {
		switch (o) {
		case HORIZONTAL:
	 ...
	 *            The orientation.
	 */
	private static void translateRightAligned(final Graphics2D g, final RectangularShape rect, final Orientation o) {
		// FIXME bug #44
		switch (o) {
		case HORIZONTAL:
	 ...
	 *            The rectangle.
	 */
	public static final void drawAtCenter(final Graphics2D g, final String text, final RectangularShape rect) {
	 ...
	 *            The position.
	 */
	public static final void drawCentered(final Graphics2D g, final String text, final Point2D pos) {
		final StringDrawer sd = new StringDrawer(g, text);
	 ...
	 * @see #BOTTOM
	 */
	public static final void drawText(final Graphics2D g, final String text, final Point2D pos, final int hAlign,
			final int vAlign) {
		final StringDrawer sd = new StringDrawer(g, text);
