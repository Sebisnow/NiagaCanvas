...
public class SimpleNodeLinkView<NiagarinoOperators> implements NodeLinkView<NiagarinoOperators> {
	...
	/**
	 * The node that is primarily selected at the moment
	 */
	private NiagarinoOperators primSel;

	/**
	 * The node that is secondarily selected at the moment
	 */
	private NiagarinoOperators secSel;
	...
	public void addEdge(final NiagarinoOperators from, final NiagarinoOperators to) {
		...
		if (!areConnected(getId(from), getId(to))) {
			if (from instanceof Operator && to instanceof Operator && to != from) {
				...
				if (this.addEdge(getId(from), getId(to))) {
					((Stream) from).getChildIDs().add(((Stream) to));
					((Stream) to).getParentIDs().add(((Stream) from));
	...
	public boolean checkNodeProperties(NiagarinoOperators from1, NiagarinoOperators to1) {
		if (from1 instanceof Operator && to1 instanceof Operator) {
			...
			System.out.println("Trying to connect two Streams");
			for (Stream stream : from.getChildIDs()) {
				if (stream.getID() == from.getID()) {
					// Prevent Streams from being connected in both directions.
					System.out.println("interconnection of Streams");
					return false;
				}
			}
			for (Stream stream : to.getParentIDs()) {
				if (stream.getID() == to.getID()) {
					// Prevent Streams from being connected in both directions.
					System.out.println("interconnection of Streams");
					return false;
				}
	...
	public void removeEdge(final NiagarinoOperators from, final NiagarinoOperators to) {
		...
		if (from instanceof Operator && to instanceof Operator) {
			removeEdge(this.getId(from), this.getId(to));
			...
			removeEdge(this.getId(from), this.getId(to));
			((Stream) from).removeChild((Stream) to);
			((Stream) to).removeParent((Stream) from);
		}
	}
	...
	public void removeNode(NiagarinoOperators node) {
		...
		if (node instanceof Operator) {
			...
			this.delDependencies((Operator) node);
		} else if (node instanceof Stream) {
			// System.out.println(((Stream) node).getOperatorList());
			this.delDependencies((Stream) node);
			for (Operator op : new ArrayList<Operator>(((Stream) node).getOperatorList())) {
	 ...
	 *            dependencies.
	 */
	public void delDependencies(Operator node) {
		int inval = niagaCanvas.NiagarinoOperators.INVALID;
		if (node.getChildID() != inval) {
			this.removeEdge(node.getID(), node.getChildID());
			((Operator) this.getNode(node.getChildID())).setParentID(inval);
		}
		if (node.getParentID() != inval) {
			this.removeEdge(node.getParentID(), node.getID());
			((Operator) this.getNode(node.getParentID())).setChildID(inval);
		}
	}
	 ...
	 *            dependencies.
	 */
	public void delDependencies(Stream node) {
		for (Stream stream : node.getChildIDs()) {
			this.removeEdge(node.getID(), stream.getID());
			stream.removeParent(node);
		}
		for (Stream stream : node.getParentIDs()) {
			this.removeEdge(stream.getID(), node.getID());
	 ...
	 * @return the selected
	 */
	public NiagarinoOperators getPrimSel() {
		return this.primSel;
	}
	 ...
	 *            the selected to set
	 */
	public void setPrimSel(NiagarinoOperators selected) {
		this.primSel = selected;
	}
	 ...
	 * @return the selected
	 */
	public NiagarinoOperators getSecSel() {
		return this.secSel;
	}
	 ...
	 *            the selected to set
	 */
	public void setSecSel(NiagarinoOperators selected) {
