...
public final class PaintUtil {
	...
	public static Rectangle2D addPadding(final RectangularShape rect, final double padding) {
		...
		return new Rectangle2D.Double(rect.getX() - padding, rect.getY() - padding, rect.getWidth() + p2,
				rect.getHeight() + p2);
	}
	 ...
	 * 
	 * @param rect
	 *            The rectangle. The result will also be stored in the
	 *            rectangle.
	 * @param padding
	 *            The padding.
	 */
	public static void addPaddingInplace(final RectangularShape rect, final double padding) {
		final double p2 = padding * 2;
		rect.setFrame(rect.getX() - padding, rect.getY() - padding, rect.getWidth() + p2, rect.getHeight() + p2);
	}
	...
	/**
	 * Encloses the given rectangle with a rounded rectangle whose rounded
	 * corners touch the corners of the rectangle.
	 * 
	 * @param dest
	 *            The destination rectangle.
	 * @param rect
	 *            The rectangle to enclose.
	 * @param border
	 *            The size of the border.
	 */
	public static void encloseRect(final RoundRectangle2D dest, final RectangularShape rect, final double border) {
		final double b2 = border * 2;
		final double arc = border / RADIUS_FACTOR;
		dest.setRoundRect(rect.getX() - border, rect.getY() - border, rect.getWidth() + b2, rect.getHeight() + b2, arc,
				arc);
	}
	 ...
	 * 
	 * @param rect
	 *            The rounded rectangle to alter.
	 * @param border
	 *            The border. The arc touches the corner of a rectangle that
	...
	public static void setArc(final RoundRectangle2D rect, final double border) {
		final double arc = border / RADIUS_FACTOR;
		rect.setRoundRect(rect.getX(), rect.getY(), rect.getWidth(), rect.getHeight(), arc, arc);
	}
	...
	public static void addPaddingInplace(final RoundRectangle2D rect, final double padding) {
		...
		rect.setRoundRect(rect.getX() - padding, rect.getY() - padding, rect.getWidth() + p2, rect.getHeight() + p2,
	 ...
	 *            The rectangle.
	 * @param radius
	 *            The radius of the round rectangle.
	 * @return The round rectangle.
	 */
	public static RoundRectangle2D toRoundRectangle(final RectangularShape rect, final double radius) {
		final double r2 = radius * 2;
		return new RoundRectangle2D.Double(rect.getX() - radius, rect.getY() - radius, rect.getWidth() + r2,
				rect.getHeight() + r2, radius, radius);
	}
	...
	public static Rectangle2D scaleCenter(final RectangularShape rect, final double scale) {
		...
		return new Rectangle2D.Double(rect.getCenterX() - w * 0.5, rect.getCenterY() - h * 0.5, w, h);
	}
	 ...
	 *            The rectangle.
	 * @param w
	 *            The width for defining the aspect ratio.
	 * @param h
	 *            The height for defining the aspect ratio.
	 * @return A rectangle with the given aspect ratio that fits into the given
	 *         rectangle.
	 */
	public static Rectangle2D fitInto(final RectangularShape rect, final double w, final double h) {
	 ...
	 */
	public static double fitIntoScale(final RectangularShape rect, final double w, final double h) {
	 ...
	 * @param fit
	 *            If set to <code>true</code> the rectangle will be completely
	 *            visible. When set to <code>false</code> as much as possible
	 *            from the rectangle will be visible without showing anything
	 *            else.
	 * @return The scaling to apply.
	 */
	public static double fitIntoPixelScale(final int pixWidth, final int pixHeight, final double w, final double h,
			final boolean fit) {
	 ...
	 */
	public static Shape createLine(final double x1, final double y1, final double x2, final double y2,
			final double width) {
		final Point2D ortho = VecUtil.setLength(new Point2D.Double(y1 - y2, x2 - x1), width * 0.5);
		...
		gp.lineTo(x1 + ortho.getX(), y1 + ortho.getY());
		// TODO from here is just a test --> mies gehackt should belong into
		// edge arrow realizer
		/**
		 * final Point2D pos = new Point2D.Double(x2, y2); final Point2D
		 * posBefore = new Point2D.Double(x1, y1); final Point2D newPos =
		 * VecUtil.interpolate(pos, posBefore, 0.1); // TODO get vector go
		 * radius back and draw arrow there!
		 * 
		 * final double radius = RectangularNodeRealizer.RADIUS; final Point2D
		 * side = getOrthoRight(pos); final Point2D left = new
		 * Point2D.Double(newPos.getX() - 50, newPos.getY() - 50); final Point2D
		 * right = new Point2D.Double(newPos.getX() + 50, newPos.getY() + 50);
		 * gp.moveTo(newPos.getX(), newPos.getY()); gp.lineTo(left.getX(),
		 * left.getY()); gp.lineTo(right.getX(), right.getY());
		 * gp.lineTo(newPos.getX(), newPos.getY());
		 * 
		 * // final Point2D posRadius = VecUtil.interpolate(pos, new //
		 * Point2D.Double(x1, y1), 0.2); // gp.moveTo(pos.getX() - ortho.getX(),
		 * pos.getY() - ortho.getY()); // gp.lineTo(pos1.getX() + ortho.getX(),
		 * pos1.getY() + ortho.getY()); // gp.lineTo(pos1.getX() - ortho.getX(),
		 * pos1.getY() - ortho.getY()); // gp.lineTo(x2 - ortho.getX(), y2 -
		 * ortho.getY());
		 * 
		 * // gp.lineTo(pos2.getX() - ortho.getX(), pos2.getY() - ortho.getY());
		 * // gp.lineTo(pos2.getX() + ortho.getX(), pos2.getY() + ortho.getY());
		 * // gp.lineTo(x2 + ortho.getX(), y2 + ortho.getY());
		 * 
	 ...
	 * @param t
	 *            The interpolation value from <code>0</code> to <code>1</code>.
	 ...
	 *            The color.
	 * @param alpha
	 *            The alpha value to multiply to the previous value.
	 ...
	 * 
	 * @param h
	 *            The hue value. <code>1</code> represents <code>360</code>
	 *            degrees.
	 * @param s
	 ...
	 * @return The color.
	 */
	public static Color hsbaColor(final double h, final double s, final double b, final double alpha) {
	 ...
	 *            The color.
	 * @param alpha
	 *            An optional array with a length of at least one that is used
	 ...
	 *            The border color.
	 * @param fill
	 *            The color used to fill and used when the zoom level is to
	 *            small.
	 */
	public static void drawShape(final Graphics2D g, final Shape shape, final KanvasContext ctx, final Color border,
			final Color fill) {
	 ...
	 *            small.
	 */
	public static void drawShape(final Graphics2D g, final Shape shape, final double zoom, final Color border,
			final Color fill) {
		if (fill != null) {
			g.setColor(fill);
			g.fill(shape);
	 ...
	 *            The context.
	 * @param near
	 *            The color used when being near.
	 * @param far
	 *            The color used when being far.
	 */
	public static void fillShape(final Graphics2D g, final Shape shape, final KanvasContext ctx, final Color near,
			final Color far) {
	 ...
	 *            The color used when being far.
	 */
	public static void fillShape(final Graphics2D g, final Shape shape, final double zoom, final Color near,
			final Color far) {
		final double ratio = DISSAPPEAR / zoom;
		if (ratio >= 1) {
			g.setColor(far);
	...
	public static Color getFontColor(final Color back) {
		...
		if (0.2126 * rgb[0] + 0.7152 * rgb[1] + 0.0722 * rgb[2] > 0.5)
			return Color.BLACK;
		return Color.WHITE;
	}
